...
export function AgriculteurArchivedTableRow({ agriculteur, onReactivate, onSelect }: Props) {
  ...
  return (
    <tr
      className="agri-row--archived agri-row--clickable"
    ...
    >
      ...
      <td>
        <div className="agri-actions">
          <button className="agri-action-btn agri-action-btn--reactivate" title="Reactiver" onClick={(event) => { event.stopPropagation(); onReactivate(agriculteur) }}><RotateCcw size={14} /></button>
        </div>
      </td>
    </tr>
  )
